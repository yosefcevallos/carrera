...
| `ExitRequested` | `5c7d0617e9c49546` | vault, user, shares u64, epoch_id u64 |
| `ExitCancelled` | `58d40495ac4089e8` | vault, user, shares u64 |
...
| `Redeemed` | `0e1db7471fa56b26` | vault, user, shares u64, stock u64, usdc u64 |
...
| `NavRefreshed` | `82ea401109eea128` | vault, nav_usd_e6 u64, share_price_stock_e6 u64 |
...
| Event | Table | Note |
|---|---|---|
| `NavRefreshed` | `nav_samples` | `price_e6` is not in the event; left null |
...
| `ExitRequested` | `exits` | keyed by request tx signature, since the event has no nonce |
| `ExitCancelled` / `Redeemed` | `exits` | matched to the oldest open request for (vault, user) with equal `shares` |
| everything | `program_events` | raw, JSON payload, unique on (signature, log_index) |

## Requests to the program (would make indexing exact)

1. Add `nonce: u64` to `ExitRequested`, `ExitCancelled` and `Redeemed` so exits key on `(vault, user, nonce)`.
2. Add `price_e6: u64` to `NavRefreshed` so `nav_samples.price_e6` is populated from chain.
